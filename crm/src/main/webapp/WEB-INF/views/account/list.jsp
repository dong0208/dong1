	...
	$("#editAccountForm").validate({
		...
		$(document).delegate(".del","click",function(){
			...
			$.ajax({
		        ...
		        success: function (data) {
			        if(data.state == 'success') {
			        	layer.alert("删除完成");
			        	pageload(deptId);      //删除完成之后直接显示删除后的页面
		        	}
		        }  
		    });
		...
		$(document).delegate(".del","click",function(){ 
			...
			layer.confirm("确定要删除么？",function(){
				layer.close();
				location.href="/account/del?id="+ id;
			})
		});	
		
		
		$(document).delegate(".de","click",function(){
			var id = $(this).attr("rel");
			$.ajax({
		        url:"/list",
		        type:"post",
			...
			$.ajax({
				...
				success:function(data){
					...
					for (var i = 0; i < page.items.length; i++) {
		            	...
		            	var html = "<tr> <td>" + account.username+"</td> <td>" + account.deptName + "</td> <td>"+account.mobile+"</td> <td> <a href='javascript:;' class='del' rel='" + account.id + "'> 删除</a><a href='javascript:;' class='edit' rel='" + account.id + "'> 编辑</a></td></tr>";
		            	console.log($(html)[0]);
		            	$("#body").append(html);
					}
				}
